...
class VGGSfM(nn.Module):
    def __init__(self, TRACK: Dict, CAMERA: Dict, TRIANGULAE: Dict, cfg=None):
        ...
        self.cfg = cfg
        self.device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')#this was added
    ...
    def from_pretrained(self, model_name):
        try:
            ...
            #_VGGSFM_URL = "https://huggingface.co/facebook/VGGSfM/resolve/main/vggsfm_v2_0_0.bin" #changed
            ...
            checkpoint = torch.hub.load_state_dict_from_url(_VGGSFM_URL, map_location=torch.device('cpu'))
